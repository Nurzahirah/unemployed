...
yr <- c(1990,1991,1992,1993,1994,1995,1996,1997,1998,1999,year)
...
shinyServer(function(input, output) {
  ...
  male2 = male[sample(nrow(male), 152, replace = FALSE, prob=NULL),
               ]
  output$mytable2 <- DT::renderDataTable({
    DT::datatable(male2[, input$show_vars2, drop = FALSE])
  })
  female2 = female[sample(nrow(female), 152, replace = FALSE, prob=NULL),
                   ]
  ...
  output$fgraph <- renderPlot({
    ...
    ggplot(d,aes(x=year,y=rate))+geom_bar(fill = "#0073C2EF",stat = "identity")+xlab("year")+ylab("unemployment rate")
